...
void Run(MSG& msg, HACCEL hAccelTable);
...
void Update(HWND hWnd)
{
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
		...
		Circle newCircle(cursorPos.x , cursorPos.y);

		circleList.PushBack(newCircle);
	...
	if (GetAsyncKeyState(VK_RBUTTON) & 0x8000)
	{
		...
		LinkedList<Circle> circlesToRemove;

		Node<Circle>* nodePtr = circleList.GetHead();
		while (nodePtr)
		{
			Circle& circle = nodePtr->data;

			int distance = static_cast<int>(sqrt(pow(cursorPos.x - circle.x, 2) + pow(cursorPos.y - circle.y, 2)));

			if (distance <= 10)
			{
				circlesToRemove.PushBack(circle);
			}

			nodePtr = nodePtr->next;
		}

		Node<Circle>* removeNodePtr = circlesToRemove.GetHead();
		while (removeNodePtr)
		{
			circleList.Remove(removeNodePtr->data);
