...
void Update()
{
    ...
    if (GetAsyncKeyState(VK_RBUTTON) && 0x8000)
    {
        ...
        if (!prevState[1])
        {
            ...
            {
                int x = circles.GetNode(i)->data.x;
                int y = circles.GetNode(i)->data.y;
                if (sqrt(pow((x - pos.x), 2) + pow((y - pos.y), 2)) < 10)
                {
                    circles.Delete(circles.GetNode(i));
...
void Render()
{
    ...
    oldBrush = reinterpret_cast<HBRUSH>(SelectObject(hdc, myBrush));
